...
//! - Connection timeout: Returns PreflightError::IoError after timeout
//! - HTTP errors: Returns PreflightError::IoError with status code
//! - Missing attributes: get_instance_attribute() returns Ok(None) for 404
//! - Parse errors: Returns PreflightError::ParseError with context
//!
...
pub fn is_on_gcp() -> bool {
    ...
    TcpStream::connect_timeout(
        &format!("{}:{}", METADATA_IP, METADATA_PORT).parse().unwrap(),
        Duration::from_millis(1000),
    ).is_ok()
}

/// Get the GCP project ID
pub fn get_project_id() -> Result<String, PreflightError> {
    metadata_get("/computeMetadata/v1/project/project-id")
}

/// Get the instance zone
pub fn get_zone() -> Result<String, PreflightError> {
    ...
    zone_path
        ...
        .next()
        .map(|s| s.to_string())
        .ok_or_else(|| PreflightError::ParseError {
...
pub fn get_instance_name() -> Result<String, PreflightError> {
    metadata_get("/computeMetadata/v1/instance/name")
}

/// Get the machine type
pub fn get_machine_type() -> Result<String, PreflightError> {
...
pub fn get_service_account() -> Result<String, PreflightError> {
...
pub fn get_access_scopes() -> Result<Vec<String>, PreflightError> {
...
pub fn get_instance_attribute(attr: &str) -> Result<Option<String>, PreflightError> {
    match metadata_get(&format!("/computeMetadata/v1/instance/attributes/{}", attr)) {
        Ok(value) => Ok(Some(value)),
        Err(PreflightError::IoError { message, .. }) if message.contains("404") => Ok(None),
        Err(e) => Err(e),
    }
}

/// Make a GET request to the metadata server
fn metadata_get(path: &str) -> Result<String, PreflightError> {
    metadata_get_with_timeout(path, DEFAULT_TIMEOUT_MS)
}

/// Make a GET request to the metadata server with custom timeout
fn metadata_get_with_timeout(path: &str, timeout_ms: u64) -> Result<String, PreflightError> {
    // Connect to metadata server
    let addr = format!("{}:{}", METADATA_IP, METADATA_PORT);
    let mut stream = TcpStream::connect_timeout(
        &addr.parse().map_err(|_| PreflightError::IoError {
    ...
    loop {
        ...
        reader.read_line(&mut line).map_err(|e| PreflightError::IoError {
            context: "metadata_get".to_string(),
            message: format!("Read failed: {}", e),
        })?;
